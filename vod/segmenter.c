	...
	get_clip_ranges_result_t* result)
{
	...
	uint64_t clip_start_offset = params->start_time;
	uint64_t start;
	uint64_t end;
	uint64_t ignore;
  	...
  	for (cur_duration = params->clip_durations;; cur_duration++)
	{
        ...
        clip_start_offset += *cur_duration;
 	}
	...
	segment_durations_t* result)
{
    ...
    // this is to say: *don't scratch the end of last segment as it may be too short*
    if(!media_set->presentation_end && result->end_time <= (cur_item->duration + clip_offset + result->start_time)){
        cur_item--;
